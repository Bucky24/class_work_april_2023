...
const seedDatabase = async () => {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const randomTravellerIndex = Math.floor(Math.random() * travellers.length);
    const randomTraveller = travellers[randomTravellerIndex];
    const { id: randomTravellerId } = randomTraveller;
    ...
    await Trip.create(tripData).catch((err) => {
